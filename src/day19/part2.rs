...
fn options(blueprint: &Blueprint, state: &State, time_limit: u8) -> Vec<State> {
    ...
    let clay_condition = state.claybots * (time_limit - state.time)
        + (state.clay - state.allocated_clay(blueprint))
        < (time_limit - state.time).saturating_mul(max_claybots);

    if state.claybots < max_claybots && clay_condition {
    ...
    let obsidian_condition = state.obsidianbots * (time_limit - state.time)
        + (state.obsidian - state.allocated_obsidian(blueprint))
        < (time_limit - state.time).saturating_mul(max_obsidianbots);

    if state.claybots > 0 && state.obsidianbots < max_obsidianbots && obsidian_condition {
...
fn max_geodes_by(blueprint: &Blueprint, time_limit: u8) -> u8 {
    ...
    while let Some(mut state) = queue.pop() {
        ...
        for mut option in options(blueprint, &state, time_limit) {
            queue.push(option);
        }
...
pub fn solve(input: &Input) -> Output {
    input[0..input.len().min(3)]
        .par_iter()
        .map(|blueprint| max_geodes_by(blueprint, 32) as usize)
        .product::<usize>()
        .into()
}
